...
function isValidNeedId(value: unknown): value is string {
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const parsedNeedId =
      typeof needIdRaw === 'string' && needIdRaw.trim() ? needIdRaw.trim() : undefined;
    ...
    const needId = (needIdRaw as string).trim();
    ...
    if (!best) {
      return res.status(400).json({
        error:
          'No valid match: need must have lat/lng, and at least one available resource with lat/lng and quantity > 0.',
      });
    }
